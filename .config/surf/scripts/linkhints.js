...
function setHintRules() {
	...
	ss.insertRule('a[highlight=hint_elem] {background-color: none}', 0);
	ss.insertRule('a[highlight=hint_active] {background-color: none}', 0);
}
...
function addKeyBind(key, func, evt){
	var pressedKey = get_key(evt);
	if(pressedKey == key) {
		evt.preventDefault();  //Stop Default Event
		eval(func);
	}
}

function initKeyBind(evt) {
	var t = evt.target;
	if( t.nodeType == 1){
		addKeyBind('§', 'hintMode()', evt);
		addKeyBind('Esc', 'removeHints()', evt);
	}
}
